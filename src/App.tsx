...
import {
  parseDeclineFile, parseConnectionsFile, parseLeadsFile, detectKind,
...
export default function App() {
  ...
  const handleFiles = async (files: File[]) => {
    ...
    try {
      ...
      if (result.accounts.length === 0 && result.activity.length === 0 && result.leads.length === 0) {
        setError('No data could be parsed from the uploaded files. Check that filenames include keywords like "Decline", "Connections", or "Leads".');
        setLoading(false);
        return;
      }
